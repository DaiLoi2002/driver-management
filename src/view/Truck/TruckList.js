...
const TruckList = () => {
  ...
  const [serviceSupport, setServiceSupport] = useState([
    { serviceId: "support_001", name: "Dịch vụ bốc xếp",price:360.000, priceString: "₫360.000", percent: 0, selected: false },
    { serviceId: "support_002", name: "Giao hàng 2 chiều",price:360.000, priceString: "+70%", percent: 70, selected: false },
    { serviceId: "support_003", name: "Thùng mở / kín",price:360.000, priceString: "+10%", percent: 10, selected: false },
    { serviceId: "support_004", name: "Hỗ trợ phí cầu đường",price:360.000, priceString: "+10%", percent: 10, selected: false }
  ]);


  const [boxState, setBoxState] = useState([
    { Id: "box_001",name: "Thùng Mở",percent: 10, pricePercent: "10%", selected: false },
    { Id: "box_002",name: "Thùng Kín",percent: 10, pricePercent: "10%", selected: false },
  ...
  const [detailsUnloading, setDetailsUnloading] = useState([
    ...
    },
    {
      ...
      price: 530000,
      priceString: "₫530.000",selected: false
    },
    {
      Id: "unloading_003",
      name: "Bốc xếp Tận Nơi (Có Người Hỗ Trợ)",
      price: 580000,
      priceString: "₫580.000",selected: false
    }
  ]);
  ...
  }, [boxState, serviceSupport]); // Sẽ chạy khi boxState hoặc serviceSupport thay đổi
  ...
  const handleInputChange = (e) => {
    ...
    const updatedValue = name === 'superSpeedFare' || name === 'minFare' ? Number(value) : value;
  ...
  const handleNestedChange = (field, range, updatedFare) => {
    setNewTruck((prevNewTruck) => {
      ...
      if (field === 'dimensions' && ['length', 'width', 'height'].includes(range)) {
        updatedField.title = `${updatedField.length} x ${updatedField.width} x ${updatedField.height} Mét`;
      }
  
      if (field === 'prohibitedHours' && ['morning', 'afternoon'].includes(range)) {
  ...
  const handleUpdateTruck = (truckId) => {
    ...
    if (truckToUpdate) {
      ...
      console.log("detailsUnloading sau khi cập nhật:", updatedDetailsUnloading);
    }
  ...
  const handleCheckboxServiceChange = (serviceId) => {
    ...
    setNewTruck((prevNewTruck) => {
      const selectedService = serviceSupport.find((service) => service.serviceId === serviceId);
      ...
      if (selectedService && selectedService.serviceId) {
        ...
        const updatedServiceSupport = isSelected
          ...
          : (prevNewTruck.serviceSupport || []).filter((service) => service.serviceId !== serviceId); // Xóa dịch vụ nếu bỏ chọn
  ...
  const handleDetailUnloadingCheckboxChange = (id) => {
    ...
    setDetailsUnloading((prevDetailsUnloading) => {
      ...
      const selectedUnloading = updatedDetails.filter((unloading) => unloading.selected);
      setNewTruck((prevNewTruck) => ({
        ...prevNewTruck,
  ...
  const handleSubmit = async (e) => {
    ...
    setNewTruck({
      ...
      setServiceSupport(serviceSupport.map(service => ({
        ...service,
        selected: false // Đặt lại selected: false cho tất cả dịch vụ
      })));

      setBoxState(boxState.map(box => ({
        ...box,
        selected: false // Đặt lại selected: false cho tất cả thùng
      })));

      setDetailsUnloading(detailsUnloading.map(unloading => ({
        ...unloading,
    ...
    <div>
      ...
      {truckData && truckData.data && truckData.data.length > 0 ? (
        truckData.data.map((truck) => (
          <TruckItem key={truck.id} truck={truck} handleDeleteTruck={handleDeleteTruck} handleUpdateTruck={handleUpdateTruck} />
        ))
      ) : (
       ...
       <form onSubmit={handleSubmit}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', textAlign: 'center' }} >
          <div style={{ flex: '0 0 48%', textAlign: 'left' }} >
            <h4>Thông tin:</h4>
            <span style={{ display: 'inline-block', textAlign: 'left' }}>Tên xe</span>
            <input type="text" name="name" value={newTruck.name} onChange={handleInputChange} required />

            <span style={{ display: 'inline-block', textAlign: 'left' }}>Loại xe</span>
            <input type="text" name="type" value={newTruck.type} onChange={handleInputChange} required />

            <span style={{ display: 'inline-block', textAlign: 'left' }}>URL Ảnh</span>
            <input type="text" name="image" value={newTruck.image} onChange={handleInputChange} required />
          </div>

          <div style={{ flex: '0 0 48%', textAlign: 'left' }} >
            <h4>Kích thước:</h4>
           
            <span style={{ display: 'inline-block', textAlign: 'left' }}>Chiều dài (m)</span>
            <input type="number" name="length" value={newTruck.dimensions.length} onChange={(e) => handleNestedChange('dimensions', 'length', e.target.value)} placeholder="" />
            <span style={{ display: 'inline-block', textAlign: 'left' }}>Chiều rộng (m)</span>

            <input type="number" name="width" value={newTruck.dimensions.width} onChange={(e) => handleNestedChange('dimensions', 'width', e.target.value)} placeholder="" />
            <span style={{ display: 'inline-block', textAlign: 'left' }}>Chiều cao (m)</span>
            <input type="number" name="height" value={newTruck.dimensions.height} onChange={(e) => handleNestedChange('dimensions', 'height', e.target.value)} placeholder="" />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
          <div style={{ flex: '0 0 48%', textAlign: 'left' }} >
            ...
            <span style={{ display: 'inline-block', textAlign: 'left' }}>Khối lượng (kg)</span>
            <input type="number" name="weight" value={newTruck.maxCapacity.weight} onChange={(e) => handleNestedChange('maxCapacity', 'weight', e.target.value)} placeholder="" />



            <span style={{ display: 'inline-block', textAlign: 'left' }}>Dung tích (CBM)</span>

            <input type="number" name="volume" value={newTruck.maxCapacity.CBM} onChange={(e) => handleNestedChange('maxCapacity', 'CBM', e.target.value)} placeholder="" />
          </div>
          <div style={{ flex: '0 0 48%', textAlign: 'left' }} >

            <h4>Giờ cấm:</h4>
            <span style={{ display: 'inline-block', textAlign: 'left' }}>Sáng</span>
            <input type="text" name="morning" value={newTruck.prohibitedHours.morning} onChange={(e) => handleNestedChange('prohibitedHours', 'morning', e.target.value)} placeholder="" />
            <span style={{ display: 'inline-block', textAlign: 'left' }}>Chiều</span>
            <input type="text" name="afternoon" value={newTruck.prohibitedHours.afternoon} onChange={(e) => handleNestedChange('prohibitedHours', 'afternoon', e.target.value)} placeholder="" />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
          <div style={{ flex: '0 0 48%', textAlign: 'left' }}  >
            ...
            <span style={{ display: 'inline-block', textAlign: 'left' }}>Giá cước nhanh:</span>
            <input
              type="number"
              name="superSpeedFare"
            ...
            <span style={{ display: 'inline-block', textAlign: 'left' }}>Cước tối thiểu (VND)</span>
            <input type="number" name="minFare" value={newTruck.minFare} onChange={handleInputChange} placeholder="" required />
      ...
      {serviceSupport.map((service) => (
          ...
          >
            <input
              ...
              onChange={() => handleCheckboxServiceChange(service.serviceId)} // Gọi hàm khi nhấn checkbox
            />
            <label htmlFor={`service-${service.serviceId}`} style={{ marginLeft: '10px' }}>
              {service.name}
            </label>
    ...
    <div style={{ marginTop: '5px' }}>
     ...
     <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
            {boxState.map((box) => (
              <div key={box.Id} style={{ flex: '1 1 200px', padding: '10px', border: '1px solid #ddd', borderRadius: '5px' }}>
                ...
                />
                <label htmlFor={`service-${box.Id}`} style={{ marginLeft: '10px' }}>
                  {box.name}
                </label>
    ...
    >
      <input
        ...
        id={`unloading-${unloading.Id}`}
         onChange={() => handleDetailUnloadingCheckboxChange(unloading.Id)}
        checked={unloading.selected}
      />
      <label htmlFor={`unloading-${unloading.Id}`} style={{ marginLeft: '10px' }}>
        {unloading.name}
      </label>
      <div style={{ marginTop: '5px' }}>
        <span style={{ display: 'inline-block', width: '200px', textAlign: 'left' }}>
      ...
      <div className="truck-info">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', padding: '20px' }}>
          ...
          <div className="truck-details">
            <p><strong>Loại xe:</strong> {truck?.type || 'N/A'}</p>
            <p>
              <strong>Kích thước:</strong> 
              {truck?.dimensions?.length ? `${truck.dimensions.length}m x ${truck.dimensions.width}m x ${truck.dimensions.height}m` : 'Thông tin không có'}
            </p>
            <p>
              <strong>Tải trọng tối đa:</strong> 
              {truck?.maxCapacity?.weight ? `${truck.maxCapacity.weight} kg, ${truck.maxCapacity.CBM} CBM` : 'Thông tin không có'}
            </p>
            <p>
              <strong>Giờ cấm:</strong> 
              {truck?.prohibitedHours?.morning && truck?.prohibitedHours?.afternoon ? 
                `${truck.prohibitedHours.morning} / ${truck.prohibitedHours.afternoon}` : 'Thông tin không có'}
            </p>
            <p><strong>Giá cước nhanh:</strong> {truck?.superSpeedFareString || 'Thông tin không có'}</p>
            <p><strong>Giá cước tối thiểu:</strong> {truck?.minFareString || 'Thông tin không có'}</p>


           <p><strong>Phí bổ sung:</strong> 
  {Array.isArray(truck?.additionalFare) && truck.additionalFare.length > 0 ? 
    truck.additionalFare.map((fare, index) => (
      ...
      >
        {`Từ ${fare.minRange} đến ${fare.maxRange} km: ${fare.rateString}`}
            ...
            <p><strong>Dịch vụ hỗ trợ:</strong> 
              {Array.isArray(truck?.serviceSupport) && truck.serviceSupport.length > 0 ? 
                truck.serviceSupport.map((service, index) => (
                  <span key={index} className="service">{service.name} - {service.priceString}</span>
                )).reduce((prev, curr) => [prev, ', ', curr]) : 'Không có dịch vụ hỗ trợ'}
            </p>

            <p><strong>Chi tiết bốc dỡ:</strong> 
              {Array.isArray(truck?.detailsUnloading) && truck.detailsUnloading.length > 0 ? 
                truck.detailsUnloading.map((detail, index) => (
                  <span key={index} className="unloading">{detail.name} - {detail.priceString}</span>
                )).reduce((prev, curr) => [prev, ', ', curr]) : 'Không có chi tiết bốc dỡ'}
            </p>

            <p><strong>Loại thùng:</strong> 
              {Array.isArray(truck?.box) && truck.box.length > 0 ? 
                truck.box.map((b, index) => (
                  <span key={index} className="box">{b.name} - {b.pricePercent}</span>
                )).reduce((prev, curr) => [prev, ', ', curr]) : 'Không có loại thùng'}
            </p>
          </div>
        </div>
      ...
      <div className="truck-actions">
        ...
        <button className="btn btn-primary" onClick={() => handleUpdateTruck(truck?.id)}>
          Cập Nhật
        </button>
      </div>
    </div>
