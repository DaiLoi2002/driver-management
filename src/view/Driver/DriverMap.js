...
useEffect(() => {
  ...
  useEffect(() => {
    mapboxgl.accessToken = 'pk.eyJ1IjoiZGFpbG9pMjAwMiIsImEiOiJjbTI3eHc3aXMxaGZqMmlzZ2thaGcwNHhoIn0.HiDAtPpR-E_tnE3SI58wkQ';
          ...
          if (Array.isArray(coordinates) && coordinates.length === 2) {
            ...
            if (!isNaN(lng) && !isNaN(lat)) {
              ...
              const marker = new mapboxgl.Marker(el)
                .setLngLat([lng, lat])
                .setPopup(new mapboxgl.Popup().setHTML(`<h3>${driver.fullName}</h3><h4>ID: ${driver._id}</h4>`))

                .addTo(mapRef.current);
                
              markersRef.current.push(marker); // Lưu lại marker vào mảng
            } else {
              console.error(`Invalid coordinates for driver ID: ${driver._id} - (Lng: ${lng}, Lat: ${lat})`);
            }
          } else {
