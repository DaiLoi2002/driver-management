...
const VoucherManagement = () => {
  ...
  const [newVoucher, setNewVoucher] = useState({
    ...
    endDate: "",
    imageUrl: "",
    userGroups: "", // Thêm trường userGroups
  });
  ...
  const handleInputChange = (e) => {
    ...
    if (value === "all") {
      // Khi chọn "Tất cả", tất cả các nhóm sẽ được thêm vào mảng
      setNewVoucher((prevState) => ({
        ...prevState,
        [name]: ["free", "basic", "plus"], // Đặt tất cả nhóm vào mảng
      }));
    } else if (type === "select-multiple") {
      // Nếu chọn các nhóm riêng biệt, cập nhật mảng userGroups
      setNewVoucher((prevState) => ({
        ...prevState,
        [name]: Array.from(
