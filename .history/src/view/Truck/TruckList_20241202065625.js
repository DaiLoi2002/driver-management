...
const TruckList = () => {
  ...
  const handleUpdateTruck = (truckId) => {
    ...
    if (truckToUpdate) {
      // Đặt lại selected: false cho tất cả các box, dịch vụ và unloading
  ...
  const handleCheckboxServiceChange = (serviceId) => {
    ...
    setNewTruck((prevNewTruck) => {
      const selectedService = prevNewTruck.serviceSupport.find(
  ...
  const handleBoxCheckboxChange = (boxId) => {
    ...
    setNewTruck((prevNewTruck) => {
      const selectedBox = prevNewTruck.box.find((box) => box.Id === boxId); // Sử dụng prevNewTruck.box
